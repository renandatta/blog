...
@section('content')
    ...
    <div class="az-content-body border-top">
        <div class="row mt-3">
            <div class="col-md-6">
                <a href="{{ route('setting') }}" class="btn btn-light">Cancel</a>
            </div>
            <div class="col-md-6 text-right">
                @if ($id != 'new')
                    <button type="button" onclick="deleteSetting('{{ $id }}')" class="btn btn-danger"><i class="la la-times text-white"></i> Delete</button>
                @endif
            </div>
        </div>
        <div class="card mt-3">
            <div class="card-body">
                ...
                <form action="{{ route('setting.save', $id) }}" method="post" id="{{ $prefix }}" enctype="multipart/form-data">
                    @csrf
    ...
    <script>
        function deleteSetting(id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.post('{{ url('setting/delete') }}/' + id, {_token: '{{ csrf_token() }}', _method: 'delete'}, function () {
